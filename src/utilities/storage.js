...
const PROD_PUBLIC_BASE = 'https://files.minalgem.com'   // ✅ Updated
...
export function getAssetUrl(relativePath) {
  ...
  try {
    ...
    // Match known folders: /products/..., /avatars/..., etc.
    ...
    // fallback: just use the entire pathname
    return `${publicBase}/${url.pathname.replace(/^\//, '')}`
  } catch {
    // Not a full URL → treat as relative
    let cleanPath = relativePath.replace(/^\/uploads\//, '') // remove leading /uploads/
    cleanPath = cleanPath.replace(/^\//, '') // remove any leading slash
